...
author: "Robin Lovelace, Jakub Nowosad, James Muenchow, Document is Adapted from Geocomputation with R Book; \ Presenting - Rasha Elnimeiry with Bilikisu Wunmi Olatunji"
format: html
editor: visual
toc: TRUE
---

# Chapter 4: Spatial Data Operations {#spatial-operations}
...
```{r 04-spatial-operations-1, message=FALSE, warning=F, results='hide'}
library(sf)
library(terra)
library(tidyverse)
library(spData)
...
-   Different *types* of spatial relationship between objects, including **intersects** and **disjoint**.
...
-   Spatial operations on raster objects include subsetting, joining and aggregating.
...
```{block2 04-spatial-operations-2}
1. It is important to note that spatial operations that use two spatial objects rely on *both objects having the same coordinate reference system*, covered in more depth in Chapter \@ref(reproj-geo-data).
2. for spatial subsetting both x (target) and y (source) must be geographic objects. Specifically, objects used for spatial subsetting in this way must have the class sf or sfc.
```

## 4.2 Spatial operations on vector data {#spatial-vec}

-   spatial operations on vector geographic data represented as simple features in the **sf** package.

-   spatial operations on raster datasets using classes and functions from the **terra** package.
...
-   Spatial **subsetting** is the process of taking a spatial object and returning a new object containing only features that ***relate*** in space to another object.
    ...
    -   Analogous to *attribute subsetting* , subsets of `sf` data frames can be created with square bracket (`[`) operator using the syntax `x[y, , op = st_intersects]`,
...
1.  **Use the `[` operator (\*\* use this or number 3)**\
    `points[polygon, ,  op = st_intersects]`\
    or `points[polygon, , op = st_disjoint]`
...
3.  **Use the sf::st_filter + .predicate = subsettingfunction (\*\*use this or number 1)**
...
Goal: find how many points (nz_height) fall within the polygon (canterbury)

We have these datasets: ***nz_height*** (point), ***nz*** (polygon).\
...
First - **always** make sure both spatial objects are:

1\. spatial objects and\
2. both spatial objects are in the same crs (many ways to do this; covered in later chapters).
...
Joining two non-spatial datasets relies on a shared 'key' variable, as described in Section \@ref(vector-attribute-joining). Spatial data joining applies the same concept, but instead relies on spatial relations, described in the previous section.

-   As with attribute data, joining adds new columns to the target object (the argument `x` in joining functions), from a source object (`y`). \index{join!spatial} \index{spatial!join}
...
set.seed(2018) # set seed for reproducibility
 st_bbox(world)-> bb # the world's bounds
data.frame(
  x = runif(n = 10, min = bb[1], max = bb[3]),
  y = runif(n = 10, min = bb[2], max = bb[4])
) %>% 
  st_as_sf(coords = c("x", "y"), crs = "EPSG:4326")->random_points  # set coordinates and CRS
# 10 random points
```
...
st_join(x = random_points,left = F, y=world["name_long"])->random_joined 
random_joined %>% view
world %>% view
...
-   The resulting `nz_agg` objects have the same geometry as the aggregating object `nz` but with a new column summarizing the values of `x` in each region using the function `mean()`. Other functions could be used instead of `mean()` here, including `median()`, `sd()` and other functions that return a single value per group.

-   Note: one difference between the `aggregate()` and `group_by() %>% summarize()` approaches is:

    -   `group_by() %>% summarize()` :tresults in `NA` values for unmatching region names,

    -   `aggregate()` preserves region names.
...
agg_aw = st_interpolate_aw(smallunits["value"], largerunits, extensive = TRUE)
agg_aw$value
