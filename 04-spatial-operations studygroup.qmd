...
The `filter()` function from the **tidyverse**\index{tidyverse (package)} can also be used but this approach is more verbose, as we will see in the examples below. \index{vector!subsetting} \index{spatial!subsetting}
...
First - **always** make sure both spatial objects are 1. spatial objects and 2. both spatial objects are in the same crs (many ways to do this; covered in later chapters)
...
nz_height[canterbury, , op = st_intersects] # samething
...
nz_height[canterbury]   ->intersect #fill in missing
...
nz_height[canterbury, ] -> canterbury_height
